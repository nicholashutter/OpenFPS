...
- All log bus consumers â†’ subscribe to `LogBusFactory.main()` via `I_LogBus.subscribe(Consumer<LogEvent>)`
  (do NOT add a new logback appender; see `engine/log/README.md`)
...
- Do NOT write to `System.out` / `System.err` in production code â€" use **SLF4J** (`LoggerFactory.getLogger`). The SLF4J logger funnels into the engine's log bus (`LogBusFactory.main()`), which subscribes the on-disk `LogFileSink` automatically; do not add a logback file appender to duplicate the same events
- Do NOT use `System.Logger` or `java.util.logging` â€" they're banned in favor of SLF4J
- Do NOT bypass `LogBusFactory` to write a log line; SLF4J is the only producer, the bus is the only router, and consumers (file sink, debug overlay, future remote sink) subscribe to the bus
...
| Memory | `memory` | Phase 1.1 â€” state machine, two backends (`JvmMemoryPort`, `ZoneMemoryPort`), factory || Log | `log` | **SHIPPING.** A non-blocking ring-buffer bus (`I_LogBus`, `RingBufferLogBus`, `LogBusFactory.main()` + `LogBusFactory.subsystem(name)`) is the spine of every engine log line. Producers write through SLF4J; the `Slf4jLogBusBridge` + `LogbackBridgeBootstrap` map SLF4J calls into `LogEvent`s and a `SubsystemStateLogger` publishes subsystem state-machine transitions. `LogBusFactory.startDrainTask()` runs a daemon thread that re-reads per-subsystem logs into the main bus so every consumer (debug overlay, future remote sink) sees them in order. **On-disk sink:** `LogFileSink` subscribes to the main bus, queues events on a bounded `ArrayBlockingQueue`, and writes them via a daemon thread to a rotating `logs/openfps-<timestamp>.log` (default 5 MiB, 3 kept). `LogSinkPaths` resolves the path from `-Dopenfps.log.file` → `OPENFPS_LOG_FILE` env → `<projectRoot>/logs/`. A burst that overflows the queue drops events and increments `droppedCount()`; the dropped counter is the only way silent loss becomes observable. Bootstrapped from `EngineMain.main()` and `DesktopLauncher.main()`; closed on `EngineSession.stop()`. Pass 9. |
| HAL | `hal` | Ports + `nulladapter` + `sqlite` + `desktop` (time, datagram) adapters, system info, user profile. Also `GameAction` / `InputBinding` / `ActionBindings` â€" the controls table. The engine owns which actions exist and deliberately ships **no default key codes**: a key code is a platform number and `:engine` may not import a toolkit that defines one, so each platform supplies its own table || HAL | `hal` | Ports + `nulladapter` + `sqlite` + `desktop` (time, datagram) adapters, system info, user profile. Also `GameAction` / `InputBinding` / `ActionBindings` â€” the controls table. The engine owns which actions exist and deliberately ships **no default key codes**: a key code is a platform number and `:engine` may not import a toolkit that defines one, so each platform supplies its own table |
...
**2679 tests passing** (1861 `:engine`, 323 `:gdxshared`, 177 `:android`,
176 `:desktop`, 142 `:tools`), Checkstyle clean — see `BUILD.md` for run
